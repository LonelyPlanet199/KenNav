# KenNav 导航页
修改说明：
1.KenNav很简单好用，对如下功能进行了改进。
2.导航链接设置为在新标签页打开，而不是在原标签，从而增加了多任务访问的方式。
3.增加了bookmark-extension的一些CSS功能。
4.增加了“设置”中导入浏览器书签文件的功能。建议先对书签文件进行分类后导入。
5.增加了定制修改背景图的功能，请在设置里增加图床链接。（因为设置成暗模式，有层暗层模版，加载图床图片后被遮盖。该功能有待完善。）
6.使用了暗黑模式的模版，本想设置明暗模式开关，但没成功。开关位置找不到了，抱歉。
7.开发是在KenNav原码基础上进行的微小改动，由于我是初学者，相关改动借助了Grok3强大功能。Cursor在修改过程中只充当了编辑器功能，代码设计修改都由Grok3按需要自行编写。
8.感谢原作者的初创。
9.一点遗憾：虽然原码支持桌面端和移动端。但因为设置内容均在本地，各端设置无法同步（除非用导入导出文件的方式）。
